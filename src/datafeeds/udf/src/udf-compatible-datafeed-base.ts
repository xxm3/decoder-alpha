...
export class UDFCompatibleDatafeedBase implements IExternalDatafeed, IDatafeedQuotesApi, IDatafeedChartApi {
	...
	protected constructor(datafeedURL: string, quotesProvider: IQuotesProvider, requester: Requester, updateFrequency: number = 10 * 1000,chartData:[]) {
		...
		this._configurationReadyPromise = this._requestConfiguration()
			.then((configuration: UdfCompatibleConfiguration | null) => {
				...
				if (configuration === null) {
					...
					configuration.supports_search = defaultconfiguration.supports_search;
					configuration.supports_marks = defaultconfiguration.supports_marks;
					configuration.supports_group_request = defaultconfiguration.supports_group_request;
					configuration.supported_resolutions = defaultconfiguration.supported_resolutions;
				}
	...
	public getBars(symbolInfo: LibrarySymbolInfo, resolution: ResolutionString, periodParams: PeriodParamsWithOptionalCountback, onResult: HistoryCallback, onError: ErrorCallback): void {
		console.log("enter getBars")
		if (periodParams.firstDataRequest) {
			console.log("enter if condition")
			// let jsonChartData = this._historyProvider.getData();
			let jsonChartData = this._chartData;
