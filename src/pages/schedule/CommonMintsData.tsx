    ...
    return (
            ...
            <div>
                     ...
                     <div>
                         ...
                         {<div className='flex flex-row'><b>Price : </b> {record?.price ? <div> record?.price</div> :<div onClick={(e) => record?.wlPrice ? history.push( { pathname: '/foxtoken',search: record?.wlTokenAddress }) : '' } className={'flex flex-row ml-1 ' + (record?.wlPrice ? ' cursor-pointer underline' : '') } dangerouslySetInnerHTML={{__html: record?.wlPrice ? `${record?.price.replace(/public/gi, "<br>public").replace('SOL', '')} (<img src="/assets/icons/FoxTokenLogo.svg" class="h-5 pr-1 foxImg" /> ${record?.wlPrice}) ◎` : record?.price &&  `${record?.price?.replace(/public/gi, "<br>public").replace('SOL', '')} ◎`}}></div>}</div>}
