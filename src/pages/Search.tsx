...
const Search: React.FC = () => {
	...
	const doSearch = async () => {
		try {
			...
			var datasetForChart = Array.from(
				{ length: numDaysBackGraphs },
				() => 0
			);
			for (let i = 0; i < sample.ten_day_count.length; i++) {
				var labels = [];
				labels = generateLabels();
				var idx = labels.findIndex(
					(val) => val === sample.ten_day_count[i].date
				);
						...
						>
							...
							<form onSubmit={(e) => onClick(e)}>
								...
								<div className="xs:flex items-center rounded-lg overflow-hidden px-2 py-1 justify-between">
									<IonSearchbar
										className="xs-flex text-base text-gray-400 flex-grow outline-none px-2 "
										type="text"
										value={searchText}
										onIonChange={(e) => {
									...
									<div className="xs:flex items-center px-2 rounded-lg space-x-4 mx-auto ">
										<IonButton
											className=" text-white text-base rounded-lg"
											onClick={() => onClick}
											animate-bounce
											disabled={searchText === ""}
										>
